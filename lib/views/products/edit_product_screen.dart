...
class _EditProductScreenState extends State<EditProductScreen> {
  ...
  final _patientCountCtrl = TextEditingController();
  ...
  void _init(ProductModel p) {
    if (_initialized) return;
    _nameCtrl.text = p.name;
    _descCtrl.text = p.description;
    _priceCtrl.text = p.price.toString();
    _imageCtrl.text = p.imageUrls.isNotEmpty ? p.imageUrls.first : '';
    _patientCountCtrl.text = p.patientCount.toString();
    _experienceCtrl.text = p.experience.toString();
  ...
  Future<void> _submit(ProductProvider provider) async {
    ...
    await provider.updateProduct(widget.productId, {
      ...
      'imageUrl': _imageCtrl.text.trim().isNotEmpty ? [_imageCtrl.text.trim()] : [],
      'patientCount': int.tryParse(_patientCountCtrl.text.trim()) ?? 0,
      'experience': int.tryParse(_experienceCtrl.text.trim()) ?? 0,
      'status': _isActive,
    });
    if (provider.error == null && mounted) {
      Helpers.showSnackBar(context, 'Doctor updated successfully');
      context.go('/products');
